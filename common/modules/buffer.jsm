...
var Buffer = Module("Buffer", {
    ...
    getPref: function getPref(pref, callback) {
        // God damn it.
        let contentPrefs = services.contentPrefs.QueryInterface(Ci.nsIContentPrefService);
        if (config.haveGecko("19.0a1"))
            contentPrefs.getPref(this.uri, pref, sanitizer.getContext(this.win), callback);
        else
            contentPrefs.getPref(this.uri, pref, callback);
    },
    ...
    setPref: function setPref(pref, value) {
        let contentPrefs = services.contentPrefs.QueryInterface(Ci.nsIContentPrefService);
        contentPrefs.setPref(this.uri, pref, value, sanitizer.getContext(this.win));
    },
    ...
    clearPref: function clearPref(pref) {
        let contentPrefs = services.contentPrefs.QueryInterface(Ci.nsIContentPrefService);
        contentPrefs.removePref(this.uri, pref, sanitizer.getContext(this.win));
    },
